...
class RepoRsyncEventHandler(FileSystemEventHandler):
    ...
    def _filter_out(self, event):
        return (
            Path(event.src_path).resolve() == Path(self._repo["source"]).resolve()
        ) or (
            Path(event.src_path).resolve()
            == Path(os.path.join(self._repo["source"], ".git"))
        )
...
def load_config():
    ...
    try:
        ...
        logging.error(
            f"Error: Permission denied to read the file {conf_file_abs_path}."
        )
        sys.exit(1)
    except json.JSONDecodeError:
        logging.error(
            f"Error: The file {conf_file_abs_path} does not contain valid JSON."
        )
        sys.exit(1)
...
def sync_repo(repo):
    ...
    for destination in repo["destinations"]:
        ...
        cmd = f"rsync -az --no-owner --no-group --delete {excludeParams} {repo['source']} {destination}"
